...
object `1-6 CEH와 슈퍼바이저 잡` {
    ...
    fun example33() {
        val job = GlobalScope.launch(Dispatchers.IO) {
            launch { printRandom33() }
        }
        Thread.sleep(1000L)
    }

    private suspend fun printRandom33() {
        delay(500L)
     ...
     *
     * GlobalScope 보다 권장되는 형식은 CoroutineScope 를 사용하는 거싱다.
    ...
    fun example34() {
        ...
        val job = scope.launch(Dispatchers.IO) {
            launch { printRandom34() }
        }
        Thread.sleep(1000L)
    }

    private suspend fun printRandom34() {
        delay(500L)
        println(Random.nextInt(0, 500))
    }
    ...
    fun example35() = runBlocking {
        ...
        val job = scope.launch(ceh) {
            launch { printRandom35_1() }
            launch { printRandom35_2() }
        }

        job.join()
    }

    private suspend fun printRandom35_1() {
        delay(1000L)
        println(Random.nextInt(0, 500))
    }

    private suspend fun printRandom35_2() {
        delay(500L)
     ...
     * */
    fun example36() = runBlocking {
        val ceh = CoroutineExceptionHandler { _, exception ->
            println("Something happened: $exception")
        }
        val job = launch(ceh) {
            val a = async { printRandom36_1() }
            val b = async { printRandom36_2() }
            println(a.await())
            println(b.await())
        }
        job.join()
    ...
    private suspend fun printRandom36_1(): Int {
        delay(1000L)
        return Random.nextInt(0, 500)
    }

    private suspend fun printRandom36_2(): Int {
        delay(500L)
        throw ArithmeticException()
    }
     ...
     * */
    fun example37() = runBlocking {
        val ceh = CoroutineExceptionHandler { _, exception ->
            println("Something happened: $exception")
        }
        val scope = CoroutineScope(Dispatchers.IO + SupervisorJob() + ceh)
        val job1 = scope.launch { printRandom37_1() }
        val job2 = scope.launch { printRandom37_2() }

        joinAll(job1, job2)
    }

    private suspend fun printRandom37_1() {
        delay(1000L)
        println(Random.nextInt(0, 500))
    }

    private suspend fun printRandom37_2() {
        delay(500L)
        throw ArithmeticException()
    }
    ...
    private suspend fun supervisorFunc() = supervisorScope {
        ...
        launch { printRandom38_1() }
        launch(ceh) { printRandom38_2() }
    }

    private suspend fun printRandom38_1() {
        delay(1000L)
        println(Random.nextInt(0, 500))
    }

    private suspend fun printRandom38_2() {
        delay(500L)
        throw ArithmeticException()
    }
}